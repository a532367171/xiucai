    ...
    public class UserBll
    {
        ...
        public bool HasUserName(string username,int userId=0)
        {
            var users = from n in UserDal.Instance.GetAll()
                        where n.UserName == username && n.KeyId != userId
                        select n;
            return users.Any();
        }
        ...
        public IEnumerable<Navigation> GetNavsBy(User u)
        {
            if (u != null)
            {
                ...
                var roleIds = enumerable.Any() ? enumerable.Select(n => n.KeyId).ToArray() : new[] { 0 };
                DataTable roleNavBtns = RoleDal.Instance.GetNavBtnsBy(roleIds);
                navArr = roleNavBtns.AsEnumerable().Select(n => (int)n["navid"]).ToArray();
        ...
        public string GetNavJson(string username)
        {
            ...
            return navigations.Any() ? JSONhelper.ToJson(BuildNavJson(navigations, 0) ): "";
        }

        private IEnumerable<object> BuildNavJson(IEnumerable<Navigation> navs,int parentId)
        {
            var navigations = navs as Navigation[] ?? navs.ToArray();
            var menus = from n in navigations
                        where n.ParentID == parentId && n.IsVisible
                        orderby n.Sortnum
                        select new
                                   {
                                       id = n.KeyId,
                                       text = n.NavTitle, n.iconCls,
                                       attributes = new
                                                        {
                                                            url = n.Linkurl, n.iconUrl,
                                                            parentid = n.ParentID,
                                                            sortnum = n.Sortnum,
                                                            n.BigImageUrl
                                                        },
                                        children = BuildNavJson(navigations,n.KeyId)
                                   };
            return menus;
        }
            ...
            if(user!=null)
            {
                ...
                return
                    userNavBtns.AsEnumerable().Select(
                        row => (int)row["navid"] == navId && (int)row["btnid"] == browser).Any();
            }
        ...
        public bool HasButton(int userId,int navId,int btnId)
        {
            ...
            if (canBrowser)
                return
                    userNavBtns.AsEnumerable().Select(row => (int) row["navid"] == navId && (int) row["btnid"] == btnId)
                        .Any();
            return false;
        }
